  ...
  function include(file, text, data) {
    ...
    function includeHandler(inst) {
      ...
      if (args) {
        ...
        var jsonContent = {};
        // import json file or read json output from function call ?
        if (args[3]) {
          if (args[3].match(/^('|")[^']|[^"]('|")$/)) {
            // import json file or read json output from function call ?
            ...
            if (fs.existsSync(jsonfile)) {
              var jsonFileRawContent = fs.readFileSync(jsonfile, 'utf-8')
              try {
                jsonContent = JSON.parse(jsonFileRawContent)
              } catch (err) {
                return console.error(err, jsonFileRawContent)
              }
            } else {
              return console.error('JSON file does not exist:', jsonfile)
            }
          } else {
            try {
              jsonContent = JSON.parse(args[3])
            } catch (err) {
        ...
        recFile = include(recFile, includeContent, jsonContent)
