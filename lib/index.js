  ...
  function include(file, text, data) {
    ...
    function includeHandler(inst) {
      ...
      if (args) {
        ...
        // handle a filename being passed instead of a json object
        if (!args[3]) {
          args[3] = /[^)"']*["']([^"']*)["'](,\s*([\s\S]*())){0,1}\s*/.exec(inst.args)[3];
        ...
        recFile = include(recFile, includeContent, args[3] ? args[3] : {})
